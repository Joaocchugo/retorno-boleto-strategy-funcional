    ...
    public static List<Boleto> lerBancoBradesco(URI caminhoArquivo) {
        try {
            ...
            for (String linha : listaLinhas) {
                ...
                boleto.setId(Integer.parseInt(vetor[0]));
                boleto.setCodBanco(vetor[1]);
                boleto.setAgencia(vetor[2]);  // Agência onde o boleto foi pago
                boleto.setContaBancaria(vetor[3]); // Conta do cliente para estorno
                boleto.setDataVencimento(LocalDate.parse(vetor[4], FORMATO_DATA));
                boleto.setDataPagamento(LocalDateTime.parse(vetor[5], FORMATO_DATA_HORA));
                boleto.setCpfCliente(vetor[6].replace(".", "").replace("-", "")); // Remove formatação do CPF
